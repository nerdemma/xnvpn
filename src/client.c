...
int main(int argc, char *argv[])
{
    ...
    // Declaración única de buffers y estructuras
    ...
    if (system(cmd) != 0)
    {
        perror("Configuracion de interfaz fallida");
        exit(1);
    }
    ...
    printf("[+] Conectado (UDP) al servidor %s\n", argv[1]);
    ...
    while (poll(fds, 2, -1) > 0)
    {
        ...
        // SOCKET -> TUN (Descifrar e inyectar)
        if (fds[1].revents & POLLIN)
        {
            ...
            if (nread > 0)
            {
                ...
                {
                    printf("[-] Error de autenticacion en el paquete entrante \n");
                }
            }
        }
    }
