    ...
    while (poll(fds, 2, -1) > 0) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            if (nread > 0 && client_connected) {
                sendto(server_fd, buffer, nread, 0, (struct sockaddr*)&client_addr, client_len);
            }
        }
        
        // Datos entrantes desde el cliente remoto hacia el túnel
        if (fds[1].revents & POLLIN) {
            ssize_t nread = recvfrom(server_fd, buffer, sizeof(buffer), 0, (struct sockaddr*)&client_addr, &client_len);
            if (nread > 0) {
                if (!client_connected) {
                    printf("[+] Cliente remoto detectado desde %s:%d\n", 
                           inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
                    client_connected = 1;
                    if(nread == 4 && memcmp(buffer, "PING", 4) == 0){continue;}
